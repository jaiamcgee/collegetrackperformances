...
SCHOOLS = {
    'Clemson': 'SC_college_f_Clemson',
    'Duke': 'NC_college_f_Duke',
    'Florida State': 'FL_college_f_Florida_State',
    'Virginia': 'VA_college_f_Virginia',
    'Stanford': 'CA_college_f_Stanford',
}
...
for school, slug in SCHOOLS.items():
    ...
    for heading in soup.find_all('h3'):
        ...
        for item in section.select('.performance-list-row'):
            ...
            school_rows.append(dict(athlete=athlete, school=school, conference='ACC',
                event=event, season=YEAR, class_year=values.get('Year',''),
                birth_date='', age_at_meet='', performance=performance,
                unit='m' if event == 'Long Jump' else 's', meet=meet,
...
out = Path('data/processed') / f'acc_{YEAR}.csv'
