#!/usr/bin/env python3
"""Preserve sourced SEC rows absent from the TFRRS rebuild.

Run after build_sec_all_meets.py for each year. The baseline is the repository
revision before the 2016-2026 SEC replacement; its rows remain in Git history.
"""
import csv
import re
import subprocess
...
BASELINE = '60e8ece4be8dd08ec3618a322427c1170ef4897a'
SCHOOLS = ['Alabama', 'Florida', 'Georgia', 'South Carolina', 'Kentucky']
...
for year in range(2016, 2027):
    path = Path(f'data/processed/sec_{year}.csv')
    with path.open(newline='', encoding='utf-8') as f:
        rows = list(csv.DictReader(f))
    baseline_path = f'data/processed/sec_{year}.csv'
    try:
...
report = Path('data/processed/sec_2016_2026_audit.csv')
